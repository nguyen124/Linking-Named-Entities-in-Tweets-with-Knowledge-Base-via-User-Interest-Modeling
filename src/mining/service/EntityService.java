...
public class EntityService {
	...
	public static Entity buildEntity(String en) {
		...
		try {
			...
			arr.addAll(candidates);
			entity.setCandidates(arr);
		} catch (Exception e) {
	...
	public static String getContext(String candidateEntity) {
		candidateEntity = candidateEntity.replaceAll("%28", "(")
				.replaceAll("%29", ")").replaceAll("_", " ")
			...
			if (data != null) {
				for (int i = 0; i < data.length(); i++) {
					...
					if (json != null) {
						...
						String temp = json.getString("snippet").replace(
								"<span class=\"searchmatch\">", "");
						temp = temp.replace("</span>", "").replaceAll(
								"[^A-Za-z0-9 ]", "");
						/*
